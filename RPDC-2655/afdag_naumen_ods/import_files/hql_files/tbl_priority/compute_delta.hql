FROM (SELECT *, 
                ...
                (SELECT 
                    ...
                    id, removal_date, removed, code, color, folder, pos, priority_level, parent, title_en, title_client, title
                FROM (SELECT coalesce(mirror.nk, reflect("java.util.UUID", "randomUUID")) AS nk,
                             ...
                             CASE
                                 ...
                                 WHEN (src.id is null and mirror.deleted_flag = 'N')
                                     THEN 'D'
                                 WHEN (mirror.deleted_flag = 'N') and ((src.removal_date <=> mirror.removal_date) = FALSE or (src.removed <=> mirror.removed) = FALSE or (src.code <=> mirror.code) = FALSE or (src.color <=> mirror.color) = FALSE or (src.folder <=> mirror.folder) = FALSE or (src.pos <=> mirror.pos) = FALSE or (src.priority_level <=> mirror.priority_level) = FALSE or (src.parent <=> mirror.parent) = FALSE or (src.title_en <=> mirror.title_en) = FALSE or (src.title_client <=> mirror.title_client) = FALSE or (src.title <=> mirror.title) = FALSE) 
                                     THEN 'U'
                                 ELSE ''
                             END                                 AS dws_act,
        ...
        , 
                     CASE
                         WHEN (src.id is not null)
                             THEN src.priority_level
                         WHEN (mirror.id is not null)
                             THEN mirror.priority_level
                         ELSE null
        ...
        , 
                     ...
                     END AS title
                      ...
                      FROM ${ods_schema_name}.tbl_priority_src AS src
                      FULL OUTER JOIN ${ods_schema_name}.tbl_priority 
                ...
                INSERT INTO TABLE ${ods_schema_name}.tbl_priority_double
                    SELECT dws_job,
