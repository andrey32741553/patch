INSERT OVERWRITE TABLE ${ods_schema_name}.tbl_servicec_configit
SELECT dws_job,
       insert_date,
       deleted_flag,
       servicecall_id,
...
FROM (SELECT *, row_number() over (partition by servicecall_id order by insert_date desc) as row_num
      FROM (
               SELECT dws_job,
                      ...
                      servicecall_id,
                      configitems_id
               FROM ${ods_schema_name}.tbl_servicec_configit
               UNION ALL
               SELECT '${dws_job}'                                    as dws_job,
                      ...
                      servicecall_id,
                      configitems_id
               FROM ${ods_schema_name}.tbl_servicec_configit_delta
