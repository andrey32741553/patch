FROM (SELECT *, 
                ...
                (SELECT 
                    ...
                    id, removal_date, removed, description, status, activecopy_id, code, color, folder, pos, parent, title_en, title_client, title
                FROM (SELECT coalesce(mirror.nk, reflect("java.util.UUID", "randomUUID")) AS nk,
                             ...
                             CASE
                                 ...
                                 WHEN (src.id is null and mirror.deleted_flag = 'N')
                                     THEN 'D'
                                 WHEN (mirror.deleted_flag = 'N') and ((src.removal_date <=> mirror.removal_date) = FALSE or (src.removed <=> mirror.removed) = FALSE or (src.description <=> mirror.description) = FALSE or (src.status <=> mirror.status) = FALSE or (src.activecopy_id <=> mirror.activecopy_id) = FALSE or (src.code <=> mirror.code) = FALSE or (src.color <=> mirror.color) = FALSE or (src.folder <=> mirror.folder) = FALSE or (src.pos <=> mirror.pos) = FALSE or (src.parent <=> mirror.parent) = FALSE or (src.title_en <=> mirror.title_en) = FALSE or (src.title_client <=> mirror.title_client) = FALSE or (src.title <=> mirror.title) = FALSE) 
                                     THEN 'U'
                                 ELSE ''
                             END                                 AS dws_act,
        ...
        , 
                     CASE
                         WHEN (src.id is not null)
                             THEN src.activecopy_id
                         WHEN (mirror.id is not null)
                             THEN mirror.activecopy_id
                         ELSE null
                     END AS activecopy_id
        , 
                     CASE
                         WHEN (src.id is not null)
                             THEN src.code
        ...
        , 
                     ...
                     END AS title
                      ...
                      FROM ${ods_schema_name}.tbl_servicetime_src AS src
                      FULL OUTER JOIN ${ods_schema_name}.tbl_servicetime 
                      AS mirror ON (src.id = mirror.id)) delta WHERE delta.dws_act <> "") delta_num ) rsl

                INSERT INTO TABLE ${ods_schema_name}.tbl_servicetime_delta partition (`date`)
                    SELECT dws_job, dws_act, nk, dws_uniact,
                    id, removal_date, removed, description, status, activecopy_id, code, color, folder, pos, parent, title_en, title_client, title, 
                    insert_date, '${date}' as `date`
                WHERE row_num = 1

                INSERT INTO TABLE ${ods_schema_name}.tbl_servicetime_double
                    SELECT dws_job,
