FROM (SELECT *, 
        ...
        , 
                     ...
                     END AS title
                      ...
                      FROM ${ods_schema_name}.tbl_mark_src AS src
                      FULL OUTER JOIN ${ods_schema_name}.tbl_mark 
                      AS mirror ON (src.id = mirror.id)) delta WHERE delta.dws_act <> "") delta_num ) rsl

                INSERT INTO TABLE ${ods_schema_name}.tbl_mark_delta partition (`date`)
                    SELECT dws_job, dws_act, nk, dws_uniact,
                    id, removal_date, removed, code, color, folder, pos, parent, title_en, title_client, title, 
                    insert_date, '${date}' as `date`
                WHERE row_num = 1

                INSERT INTO TABLE ${ods_schema_name}.tbl_mark_double
                    SELECT dws_job,
